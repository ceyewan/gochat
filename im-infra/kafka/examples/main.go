...
func main() {
	...
	// 3. 创建必要的 topics（如果不存在）
	if err := kafka.CreateExampleTopics(ctx, config); err != nil {
		log.Printf("创建示例 topics 失败: %v", err)
		log.Println("请确保 Kafka 服务正在运行，或者手动创建 topics")
		log.Println("可以使用以下命令手动创建: cd /Users/harrick/CodeField/gochat/deployment/scripts && ./init-kafka-example.sh")
	}
...
func sendExampleMessages(ctx context.Context, producer kafka.Producer) {
	...
	for _, event := range events {
		...
		// 添加 trace_id
		traceCtx := clog.WithTraceID(ctx, fmt.Sprintf("trace-%s-%d", event.UserID, time.Now().UnixNano()))
