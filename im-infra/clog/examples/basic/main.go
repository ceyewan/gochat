...
func testConsoleOutput() {
	...
	logger, err := clog.New(consoleConfig)
	if err != nil {
	...
	userModule.Info("用户模块消息", clog.String("userID", "123"))
	userModule.Warn("用户模块警告", clog.String("action", "login"))

	fmt.Println("\n4. 模块日志 - Logger 实例:")
	orderModule := logger.Module("order")
	orderModule.Info("订单模块消息", clog.String("orderID", "order-456"))
	orderModule.Error("订单模块错误", clog.String("error", "payment_failed"))
	...
	fmt.Println("\n7. 链式调用 - 全局:")
	clog.C(ctx).Module("payment").Info("支付模块链式调用", clog.String("amount", "100.00"))
	clog.C(ctx).Module("notification").Warn("通知模块链式调用", clog.String("type", "email"))

	fmt.Println("\n8. Logger 实例与 Context 结合:")
	// Logger 实例需要通过全局函数来使用 context
	contextLogger := clog.WithContext(ctx)
	contextLogger.Module("auth").Info("认证模块消息", clog.String("method", "jwt"))
	contextLogger.Module("cache").Debug("缓存模块消息", clog.String("key", "user:123"))
}
...
func testJSONFileOutput() {
	...
	logger, err := clog.New(jsonConfig)
	if err != nil {
	...
	// 创建带 traceID 的 context
	ctx := context.WithValue(context.Background(), "traceID", "json-trace-67890")
	...
	// 模块日志
	apiModule := logger.Module("api")
	apiModule.Info("API 模块消息", clog.String("endpoint", "/health"))
	apiModule.Error("API 模块错误", clog.String("status", "500"))

	// Context 日志 - 使用全局函数
	clog.WithContext(ctx).Info("JSON Context 消息", clog.String("request_id", "req-123"))
	clog.WithContext(ctx).Module("database").Warn("数据库模块警告", clog.String("query", "SELECT * FROM users"))

	// 链式调用
	clog.WithContext(ctx).Module("redis").Info("Redis 模块信息", clog.String("operation", "SET"))
