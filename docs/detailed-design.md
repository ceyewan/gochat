...
4.  **Logic (持久化)**:
    ...
    *   `im-repo` 内部实现：
        ...
        *   将消息（序列化为JSON）写入 Redis 的热点消息缓存: `ZADD hot_messages:{conversation_id} {seq_id} {message_json}`。同时修剪 ZSET，只保留最近N条（如300条）。
5.  **Logic (分发决策)**:
...
#### 5.2 缓存设计 (Redis)
(同上一份文档，包含 `user_info`, `user_session`, `conv_seq`, `group_members`, `msg_dedup`, `hot_messages`, `unread` 的详细设计)
