...
#### 设计注记：为何提供 `SetTraceID`?

一个合理的设计问题是：为何 `clog` 要提供 `SetTraceID` 来“写”上下文，而不是让调用者自己管理？

答案是：**为了实现完美的封装和提供类型安全的、统一的 API**。

- **封装**: `SetTraceID` 将用于存储 `trace_id` 的 `context.key` 作为包内私有变量，避免了将这个实现细节泄露给外部。这是 Go 社区处理上下文传递的**最佳实践**。
- **API 完整性**: 提供 `SetTraceID` 和 `C` (Get) 形成了一套完整、对称的 API，所有与日志上下文相关的操作都内聚在 `clog` 包内，职责清晰。
