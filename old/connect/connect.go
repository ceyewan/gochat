...
func (c *Connect) Run() error {
	...
	clog.Module("connect").Infof("GOMAXPROCS set to: %d", runtime.NumCPU())
	...
	clog.Module("connect").Infof("WebSocket server started successfully.")
...
func (c *Connect) initInstanceID() error {
	...
	if err != nil {
		clog.Module("connect").Errorf("Failed to get local IP: %v", err)
		return err
	}
	...
	clog.Module("connect").Infof("Service ID initialized: %s", c.InstanceID)
	return nil
}

// Shutdown 优雅退出
func (c *Connect) Shutdown(ctx context.Context) error {
	clog.Module("connect").Infof("Shutting down Connect service...")

	// 关闭 RPC 服务器
	if c.rpcServer != nil {
		clog.Module("connect").Infof("Stopping RPC server...")
		c.rpcServer.GracefulStop()
		clog.Module("connect").Infof("RPC server stopped.")
	}

	// 关闭 Etcd 客户端
	clog.Module("connect").Infof("Closing Etcd client...")
	tools.CloseEtcdClient()
	clog.Module("connect").Infof("Etcd client closed.")

	clog.Module("connect").Infof("Connect service shutdown complete.")
	return nil
}
